...
class GUI(customtkinter.CTk):
    ...
    def reported(self):
        ...
        build_inf = str([os.name, platform.platform(), platform.processor(), 'ver = 0.1.5']).encode()
        h = hashlib.sha256()
        h.update(build_inf)
        ...
        self.textbox_rep.insert("0.0", h.hexdigest())
    ...
    def ram(self):
        ...
        self.ram_button = customtkinter.CTkButton(app, text="Запустить выделение RAM",
                                                   command=self.allocate_memory(1000000000))
